...
Считаем, что сервер — Ubuntu (24.04 или новее), доступ по SSH под root. Всё
ниже выполняется на сервере, кроме первого шага.
...
**Своп — обязателен на сервере с 2 ГБ памяти.** Сборка фронтенда внутри
`docker compose up --build` — самый прожорливый момент во всём развёртывании,
а рядом ещё Postgres, API, бот и второй стек (Гермес). Без свопа сборка на
таком сервере может упасть по нехватке памяти прямо посреди `docker build`.
